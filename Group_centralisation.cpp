...
int calc (vector<int> deg,vector<int> v[],vector<int> arr,int n)	//erasing edges from nodes that are not part of the dominating set
{
	cout<<"Size of dominating set: "<<deg.size()<<endl;	
	for (int i=0;i<deg.size();i++)
	{
		cout<<"Node belonging to dominating set = "<<deg[i]<<endl;
		for (int j=0;j<v[i].size();j++)
		{
			if (arr[v[i][j]]!=1)
			{
...
void m_vertexdom(vector<int> v[],vector<int> &arr,vector<int> &s,int &k,int &n)
{
	...
	calc(deg,v,arr,n);
	cout<<"size of dominating set - "<<deg.size();
	//cout<<calcdegcen(v,n);
};
int main()
{
